<h1>Vítej na táboře 2016. Využij menu vpravo</h1>
...
if ( file_exists( "../promenne.php" ) )
{
	...
	if($spojeni)
	{
            ...
            <?php
				...
				while ($mail = mysqli_fetch_array($sql))
				{
					if ( $mail['platnost'] && $mail['mail'] != '' )
					{
						...
						echo ' value="'.$mail['mail'].'">'.$mail['nickname'].' ('.$mail['name'].' '.$mail['sname'].')</option>';
					}
				}
			?>
            </select><br />
		...
		$to = $_REQUEST['for'];
		$subject = 'Vlcata 2016 - stížnost - '.$_REQUEST['subject'];
		$sender = 'From: '.toHacky( $name, $spojeni ).'<'.$send_from.'>\n';
		$after = '
...
<?php if ( date( 'Y' ) < 2016 || (date( 'Y' ) == 2016 && date( 'n' ) <= 8) ) {?>
    <div id="bottom">
    	<div id="datumChange">
