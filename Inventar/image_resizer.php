<?php
function resizeImage( $src_image, $newW = 100, $newH = 100, $des = "." )
{
	$bool = false;
	if ( file_exists( $des ) ) {
		if ( file_exists( $src_image ) ){
			...
			if ( strstr( $src_image, '.jpg' ) || strstr( $src_image, '.jpeg' ) ) {
				$source = imagecreatefromjpeg( $src_image );
			}
			else if ( strstr( $src_image, '.png' ) ) {
				$source = imagecreatefrompng( $src_image );
			}
			else if ( strstr( $src_image, '.gif' ) ) {
			...
			$newname = $des.'/'.$name_pic.'_small.'.$ext;
